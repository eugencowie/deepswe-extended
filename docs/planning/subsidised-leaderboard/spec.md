...
- Toolbar (mirrors the DeepSWE site's chrome — toggles left, dropdowns right; revised in ticket 09's grilling, replacing the earlier vendor and effort-level filters):
  ...
  - **Subscriptions picker**: one dropdown, two labelled sections — Claude (API / Pro / Max 5x / Max 20x) and ChatGPT (API / Plus / Pro 5x / Pro 20x), labels from `tier.shortLabel` — with both API boxes ticked by default and a "(2/8)" count. Each section requires at least one ticked box (the last checked item is disabled), so the picker alone can never hide a whole family; no Select all/Clear here. Models with family `none` always show regardless of this picker.
  ...
  - Semantics: union within a dropdown, intersection across controls; filters compose with sorting and never reset it. Filter state is in-memory only — URL persistence, a reset button, and a row counter were considered and deliberately left out of ticket 09.
