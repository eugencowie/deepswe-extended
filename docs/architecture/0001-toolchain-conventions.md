...
**mise tasks delegate one-for-one to `vp`.** Each task is a thin wrapper, which doubles the edit surface when a command changes. That cost is accepted because `mise tasks` is the discovery mechanism: a contributor with only mise installed can list everything the project can do.

**The Pages base path is derived at deploy time, not hardcoded.** The deploy job reads the base from `actions/configure-pages` and passes it to `vp build --base`. The repo can move or deploy anywhere without a config edit; the trade-off is that local builds use `/` and CI tests never exercise the deployed base. A root-absolute URL that 404s under `/deepswe-analysis/` therefore passes every gate except the Playwright smoke below.
