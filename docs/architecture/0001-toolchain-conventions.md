...
**The Pages base path is derived at deploy time, not hardcoded.** The deploy job reads the base from `actions/configure-pages` and passes it to `vp build --base`. The repo can move or deploy anywhere without a config edit; the trade-off is that no build before deploy uses the real Pages base (`vp run e2e` builds at `/e2e/`; `vp run ready` and a plain `vp build` use `/`). A root-absolute URL that 404s under `/deepswe-extended/` therefore passes every gate except the Playwright smoke below.
